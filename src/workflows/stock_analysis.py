...
_llm = None
_data_adapter = None
_fundamentals_analyzer = None
_technical_analyzer = None
_bull_advocate = None
_bear_advocate = None
_judge = None
_position_advisor = None


def _init_analyzers() -> None:
    """延迟初始化分析器单例"""
    global _llm, _data_adapter, _fundamentals_analyzer, _technical_analyzer, \
           _bull_advocate, _bear_advocate, _judge, _position_advisor

    if _llm is None:
        _llm = get_llm()
        _data_adapter = get_adapter()
        _fundamentals_analyzer = FundamentalsAnalyzer(_llm)
        _technical_analyzer = TechnicalAnalyzer(_llm)
        _bull_advocate = BullAdvocate(_llm)
        _bear_advocate = BearAdvocate(_llm)
        _judge = Judge(_llm)
        _position_advisor = PositionAdvisor(_llm)


# === 节点函数（模块级） ===

def node_init(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """初始化节点"""
    return init_node(state, log_folder)


def node_fundamentals(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """基本面分析节点"""
    _init_analyzers()
    ...
    stock_info = _data_adapter.get_stock_info(stock_code)
    financial_data = _data_adapter.get_stock_financial(stock_code)

    if log_folder:
        write_log(log_folder, f"股票名称: {stock_info.get('name', 'N/A')}")
        write_log(log_folder, f"行业: {stock_info.get('industry', 'N/A')}")

    result = _fundamentals_analyzer.run({
...
def node_technical(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """技术面分析节点"""
    _init_analyzers()
    stock_code = state.get("stock_code", "")
    ...
    realtime_data = _data_adapter.get_stock_realtime(stock_code)
    kline_data = _data_adapter.get_stock_kline(stock_code, days=60)

    if log_folder:
        write_log(log_folder, f"当前价格: {realtime_data.get('price', 'N/A')}")
        write_log(log_folder, f"涨跌幅: {realtime_data.get('change', 'N/A')}%")

    result = _technical_analyzer.run({
        "stock_code": stock_code,
...
def node_aggregator(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """汇总节点"""
    return aggregator_node(state, log_folder)


def node_bull(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """正方辩论节点"""
    _init_analyzers()
...
def node_bear(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """反方辩论节点"""
    _init_analyzers()
    debate_rounds = state.get("debate_rounds", 0) + 1
    ...
    argument = _bear_advocate.run({
        "stock_code": state.get("stock_code", ""),
        "fundamentals": state.get("fundamentals", {}),
        "technical": state.get("technical", {}),
        "bull_arguments": state.get("bull_arguments", []),
        "debate_rounds": state.get("debate_rounds", 0),
        "base_arguments": sell_args
    })
...
def should_continue_debate(state: StockAnalysisState, log_folder: Optional[Path] = None) -> str:
    """判断是否继续辩论"""
    debate_rounds = state.get("debate_rounds", 0)
...
def node_judge(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """决策节点"""
    return judge_node(state, log_folder)


def node_position(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """持仓建议节点"""
    _init_analyzers()
    ...
    realtime_data = _data_adapter.get_stock_realtime(state.get("stock_code", ""))
    current_price = realtime_data.get("price", 0)

    result = _position_advisor.run({
        "stock_code": state.get("stock_code", ""),
...
def node_feishu_push(state: StockAnalysisState, log_folder: Optional[Path] = None) -> dict:
    """飞书推送节点"""
...
def create_stock_analysis_workflow(log_folder: Optional[Path] = None):
    ...
    # 使用 partial 绑定 log_folder 参数
    workflow.add_node("init", partial(node_init, log_folder=log_folder))
    workflow.add_node("fundamentals", partial(node_fundamentals, log_folder=log_folder))
    workflow.add_node("technical", partial(node_technical, log_folder=log_folder))
    workflow.add_node("aggregator", partial(node_aggregator, log_folder=log_folder))
    workflow.add_node("bull", partial(node_bull, log_folder=log_folder))
    workflow.add_node("bear", partial(node_bear, log_folder=log_folder))
    workflow.add_node("judge", partial(node_judge, log_folder=log_folder))
    workflow.add_node("position", partial(node_position, log_folder=log_folder))
    workflow.add_node("push", partial(node_feishu_push, log_folder=log_folder))
    ...
    workflow.add_conditional_edges(
        "bear",
        partial(should_continue_debate, log_folder=log_folder),
        {
            "continue": "bull",
            "end": "judge"
        }
